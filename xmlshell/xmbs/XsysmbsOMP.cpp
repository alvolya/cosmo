...
//    for (auto ii=spinsprotons.begin(); ii!=spinsprotons.end(); ++ii) cerr<<ii->first<<" "<<ii->first<<endl;
    ...
    if (nrej!=0) {
        ...
        MakePartitionOMP(PP, NX, PX, rej, rmin, rmax);
    }
...
//final oscillator quanta  purge
  if ((nhwrej>0)||bhwmax) QuantaMinMaxPurge(PP,quanta, Ncore+Pcore+hwmin,Ncore+Pcore+hwmax);
     ...
     std::cout<<"There are "<<PP.size()<<" partitions"<<std::endl;
        vector<std::list<vector<spsint> > > basis_vector(PP.size());
    //    for (int i=0;i<basis_vector.size();i++) basis_vector[i].reserve(reservation);
        
        vector<Partition::iterator> iterators(PP.size());
        int iterator_count=0;
        for (Partition::iterator ip=PP.begin(); ip!=PP.end(); ip++) iterators[iterator_count++]=ip;
        
    //    for (Partition::iterator ip=PP.begin(); ip!=PP.end(); ip++) {
    //    av::status WH_status(iterators.size(),"MBS");
    size_t itCounter = 0;
    size_t MBSculm = 0;
    #pragma omp parallel for
        for (int it_count=0;it_count<iterators.size();it_count++)
        {
    //        int *mmax=new int [levels];
            //WH_status(it_count);
            vector<int> mmax(levels);
            vector<int> mmin(levels);
            vector<spsint> mystate(A);
            Partition::iterator ip=iterators[it_count];
            int MM=M;
            ...
            MM/=2;
            Partition MP(MM,levels,&(mmax[0]),&(mmin[0]));///make M projection partition
            MP.Make();
            // cout<<MP<<endl;
            // Pause();
    ...
    //#pragma omp critical
    //                {
                        basis_vector[it_count].push_back(mystate);
    //                }
                }
    ...
    #pragma omp critical
            {
                itCounter++;
                MBSculm += basis_vector[it_count].size();
                std::cout << "\rDone: " << itCounter << "/" << iterators.size() << "\t(" << double(itCounter)*100./iterators.size() << "%) MBS: " << MBSculm << "            "<< std::flush;
            }
        }
        std::cout << std::endl;
    //    for (int i=0;i<basis_vector.size();i++) basis_vector[i].shrink_to_fit();
        for (int i=0;i<basis_vector.size();i++)
        {
    //        int bsize=basis_vector[i].size();
    //        for (int j=0;j<bsize;j++) basis[const_cast<const spsint*>(&(basis_vector[i][j][0]))]=1.0;
            
            for (list<vector<spsint> >::iterator j=basis_vector[i].begin();j!=basis_vector[i].end();j++)
            {
                basis[&((*j)[0])]=1;
            }
        }
        ...
        addAttribute(xManyBody,"creator","sysmbsOMP");
